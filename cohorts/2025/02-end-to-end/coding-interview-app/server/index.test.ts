...
describe('Session Management', () => {
    let sessions: Map<string, {
      ...
      code: string;
      language: string;
      participants: Array<{ id: string; username: string; joinedAt: string }>;
      createdAt: string;
    }>;
    ...
    it('should add participant to session', () => {
        ...
        const session = {
            ...
            code: '// Code\n',
            language: 'javascript',
            participants: [] as Array<{ id: string; username: string; joinedAt: string }>,
            createdAt: new Date().toISOString(),
        };
...
describe('Language Support', () => {
    const supportedLanguages: Record<string, { image: string; filename: string; cmd: string }> = {
        php: {
            image: 'php:8.1-cli',
            filename: 'index.php',
            cmd: 'php index.php',
...
describe('Docker Integration - Code Execution', () => {
    ...
    const runCodeInDocker = (image: string, language: string, code: string): Promise<ExecResult> => {
  ...
  fmt.Println("Hello from Go")
            ...
            const result = await new Promise<ExecResult>((resolve) => {
                ...
                const proc = exec(cmd, { maxBuffer: 10 * 1024 * 1024 }, (err, stdout, stderr) => {
                    if (!timedOut) {
                        resolve({ stdout: stdout || '', stderr: stderr || '', code: err ? ((err as ExecException).code || 1) : 0, error: err || undefined });
                    }
                });
...
func main() {
  x := undefined_var
            ...
            const result = await new Promise<ExecResult>((resolve) => {
                const proc: ChildProcess = spawn('bash', ['-c', dockerCmd]);
                let stdout = '';
                let stderr = '';
                let timedOut = false;
  ...
  public static void main invalid syntax here
    ...
    describe('Docker Container Lifecycle', () => {
        testFn('should cleanup temporary files after execution', async () => {
            ...
            const result = await new Promise<ExecResult>((resolve) => {
                const proc: ChildProcess = spawn('sh', ['-c', `docker run --rm -v "${tmpDir}:/workspace" -w /workspace python:3.11-alpine python test.py`]);
                let stdout = '';
                let stderr = '';
                let timedOut = false;
        ...
        testFn('should enforce memory limits', async () => {
            ...
            const result = await new Promise<ExecResult>((resolve) => {
                const proc: ChildProcess = spawn('sh', ['-c', `docker run --rm --memory=128m python:3.11-alpine python -c '${code.replace(/'/g, "'\\''")}'`]);
                let stdout = '';
                let stderr = '';
                let timedOut = false;
        ...
        testFn('should isolate network access', async () => {
            ...
            const result = await new Promise<ExecResult>((resolve) => {
                const proc: ChildProcess = spawn('sh', ['-c', `docker run --rm --network none python:3.11-alpine python -c '${code.replace(/'/g, "'\\''")}'`]);
                let stdout = '';
                let stderr = '';
                let timedOut = false;
